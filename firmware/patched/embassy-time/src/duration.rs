...
impl Duration {
    ...
    pub const MIN: Duration = Duration { ticks: u32::MIN };
    /// The largest value that can be represented by the `Duration` type.
    pub const MAX: Duration = Duration { ticks: u32::MAX };
    ...
    /// NOTE: Delays this small may be inaccurate.
    pub const fn from_nanos(nanoseconds: u32) -> Duration {
        Duration {
    ...
    pub const fn from_secs_floor(secs: u32) -> Duration {
        Duration { ticks: secs * TICK_HZ }
    }

    /// Creates a duration from the specified number of milliseconds, rounding down.
    pub const fn from_millis_floor(millis: u32) -> Duration {
        Duration {
    ...
    /// NOTE: Delays this small may be inaccurate.
    pub const fn from_micros_floor(micros: u32) -> Duration {
        Duration {
    ...
    pub const fn try_from_secs(secs: u32) -> Option<Duration> {
    ...
    pub const fn try_from_millis(millis: u32) -> Option<Duration> {
    ...
    /// NOTE: Delays this small may be inaccurate.
    pub const fn try_from_micros(micros: u32) -> Option<Duration> {
    ...
    /// NOTE: Delays this small may be inaccurate.
    pub const fn try_from_nanos(nanoseconds: u32) -> Option<Duration> {
    ...
    /// Fails if the number of seconds is too large.
    pub const fn try_from_secs_floor(secs: u32) -> Option<Duration> {
        let Some(ticks) = secs.checked_mul(TICK_HZ) else {
            return None;
        };
        Some(Duration { ticks })
    ...
    /// Fails if the number of milliseconds is too large.
    pub const fn try_from_millis_floor(millis: u32) -> Option<Duration> {
        let Some(value) = millis.checked_mul(TICK_HZ / GCD_1K) else {
            return None;
        };
        Some(Duration {
    ...
    /// Fails if the number of microseconds is too large.
    /// NOTE: Delays this small may be inaccurate.
    pub const fn try_from_micros_floor(micros: u32) -> Option<Duration> {
        let Some(value) = micros.checked_mul(TICK_HZ / GCD_1M) else {
            return None;
        };
        Some(Duration {
    ...
    pub const fn from_hz(hz: u32) -> Duration {
        let ticks = {
            if hz >= TICK_HZ {
                1
            } else {
...
#[inline]
const fn div_ceil(num: u32, den: u32) -> u32 {
    (num + den - 1) / den
}
...
impl From<Duration> for core::time::Duration {
    ...
    fn from(value: Duration) -> Self {
        core::time::Duration::from_micros(value.as_micros().into())
    }
}
